 ...
 */
#define	BROWSER_PAGER	((gmode & MDFKEY) ? ((gmode & MDUSEANSI) ? "pico -A -k" : "pico -k") : ((gmode & MDUSEANSI) ? "pico -A" : "pico") )
...
char *dir;
{
    ...
    if(browser_title)
      ...
      sprintf(buf, "   PINE %sL", Pmaster->pine_version);
    else
      sprintf(buf,"   UW PICO(tm) %sL", (gmode&MDBRONLY) ? "BROWSER" : version);
